...
// Declarations
enum class MagneticFieldConfigs
{
  density,
  loops
};
...
void OutflowBoundary(MeshBlock *pmb, Coordinates *pcoord, AthenaArray<Real> &prim,
                     FaceField &bb, Real time, Real dt, int il, int iu, int jl, int ju, int kl, int ku,
                     int ngh);
void Radiation(MeshBlock *pmb, const Real time, const Real dt, const AthenaArray<Real> &prim,
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
  ...
  EnrollUserExplicitSourceFunction(Radiation);
  return;
}

//----------------------------------------------------------------------------------------
// Function for calculating radiative pseudo force
// S=-Lu_\nu
// L=s \Omega P (1-\sqrt{r_{ISCO}}{r}) [\frac{P}{\rho}(\Theta r \Omega)^{-2}-1+\abs{\frac{P}{\rho}(\Theta r \Omega)^{-2}-1}]^q
               ...
               AthenaArray<Real> &cons, AthenaArray<Real> &cons_scalar)
{
  ...
  const Real r_isco = m * (3 + Z2 - sqrt((3 - Z1) * (3 + Z1 + 2 * Z2)));

  // Prepare scratch arrays
  // g: covariant metric coefficients
  // gi: contravariant metric coefficients
  AthenaArray<Real> &g = pmb->ruser_meshblock_data[0];
  AthenaArray<Real> &gi = pmb->ruser_meshblock_data[1];

  Coordinates *pcoord = pmb->pcoord;

  // start to end, include ghost zone
  for (int k = pmb->ks; k <= pmb->ke; ++k)
  {
    for (int j = pmb->js; j <= pmb->je; ++j)
    {
      ...
      for (int i = pmb->is; i <= pmb->ie; ++i)
      {
        ...
        const Real &g_00 = g(I00, i);
        const Real &g_01 = g(I01, i);
        const Real &g_03 = g(I03, i);
        const Real &g_11 = g(I11, i);
        const Real &g_13 = g(I13, i);
        const Real &g_22 = g(I22, i);
        const Real &g_33 = g(I33, i);
        const Real &g00 = gi(I00, i);
        const Real &g01 = gi(I01, i);
        const Real &g11 = gi(I11, i);
        const Real &g13 = gi(I13, i);
        const Real &g22 = gi(I22, i);
        const Real &g33 = gi(I33, i);
      ...
      if (flux_radii[n] >= pcoord->x1f(is) && flux_radii[n] < pcoord->x1f(ie + 1))
      {
        ...
        for (int j = js; j <= je; ++j)
        {
          ...
          ruser_meshblock_data[3](n, j) =
              1.0 / 3.0 * std::abs(cos_theta_m - cos_theta_p) * pcoord->dx3f(ks) * (3.0 * SQR(flux_radii[n]) + SQR(a) * (SQR(cos_theta_m) + cos_theta_m * cos_theta_p + SQR(cos_theta_p)));
        }
      }
      else
      {
    ...
    for (int k = kl; k <= ku; ++k)
    {
      for (int j = jl; j <= ju; ++j)
      {
        for (int i = il; i <= iu + 1; ++i)
        {
          ...
          if (area > 0.0 && !(std::isnan(a_2_3m) || std::isnan(a_2_3p) || std::isnan(a_3_2m) || std::isnan(a_3_2p)))
          {
    ...
    for (int k = kl; k <= ku; ++k)
    {
      for (int j = jl; j <= ju + 1; ++j)
      {
        for (int i = il; i <= iu; ++i)
        {
          ...
          if (area > 0.0 && !(std::isnan(a_1_3m) || std::isnan(a_1_3p) || std::isnan(a_3_1m) || std::isnan(a_3_1p)))
          {
    ...
    for (int k = kl; k <= ku + 1; ++k)
    {
      for (int j = jl; j <= ju; ++j)
      {
        for (int i = il; i <= iu; ++i)
        {
          ...
          if (area > 0.0 && !(std::isnan(a_1_2m) || std::isnan(a_1_2p) || std::isnan(a_2_1m) || std::isnan(a_2_1p)))
          {
    ...
    for (int k = ks; k <= ke; ++k)
    {
      for (int j = js; j <= je; ++j)
      {
        ...
        for (int i = is; i <= ie; ++i)
        {
          ...
          Real tmp = g(I11, i) * SQR(uu1) + 2.0 * g(I12, i) * uu1 * uu2 + 2.0 * g(I13, i) * uu1 * uu3 + g(I22, i) * SQR(uu2) + 2.0 * g(I23, i) * uu2 * uu3 + g(I33, i) * SQR(uu3);
          ...
          for (int n = 0; n < num_flux_radii; ++n)
          {
            if (iuser_meshblock_data[0](n) == i || iuser_meshblock_data[0](n) + 1 == i)
            {
              ...
              Real t1_0 = (rho + gamma_adi / (gamma_adi - 1.0) * pgas + b_sq) * u1 * u_0 - b1 * b_0;
              Real t1_3 = (rho + gamma_adi / (gamma_adi - 1.0) * pgas + b_sq) * u1 * u_3 - b1 * b_3;
                    ...
                    int ngh)
{
  ...
  for (int k = kl; k <= ku; ++k)
  {
    for (int j = jl; j <= ju; ++j)
    {
      ...
      {
        bb.x1f(k, j, i) = (pcoord->GetFace1Area(k, j, i + 1) * bb.x1f(k, j, i + 1) - pcoord->GetFace2Area(k, j, i) * bb.x2f(k, j, i) + pcoord->GetFace2Area(k, j + 1, i) * bb.x2f(k, j + 1, i) - pcoord->GetFace3Area(k, j, i) * bb.x3f(k, j, i) + pcoord->GetFace3Area(k + 1, j, i) * bb.x3f(k + 1, j, i)) / pcoord->GetFace1Area(k, j, i);
      }
    }
  }
  return;
                     ...
                     int ngh)
{
  ...
  for (int k = kl; k <= ku; ++k)
  {
    for (int j = jl; j <= ju; ++j)
    {
      ...
      {
        bb.x1f(k, j, i) = (pcoord->GetFace1Area(k, j, i - 1) * bb.x1f(k, j, i - 1) + pcoord->GetFace2Area(k, j, i - 1) * bb.x2f(k, j, i - 1) - pcoord->GetFace2Area(k, j + 1, i - 1) * bb.x2f(k, j + 1, i - 1) + pcoord->GetFace3Area(k, j, i - 1) * bb.x3f(k, j, i - 1) - pcoord->GetFace3Area(k + 1, j, i - 1) * bb.x3f(k + 1, j, i - 1)) / pcoord->GetFace1Area(k, j, i);
      }
    }
  }
  return;
...
namespace
{
  Real CalculateLFromRPeak(Real r)
  {
    ...
    if (prograde)
    {
      num = SQR(SQR(r)) + SQR(a * r) - 2.0 * m * SQR(a) * r - a * (SQR(r) - SQR(a)) * std::sqrt(m * r);
...
namespace
{
  Real IntegratedA1(Real x1_m, Real x1_p, Real x2, Real x3)
  {
    ...
    if (field_config == MagneticFieldConfigs::density || field_config == MagneticFieldConfigs::loops)
    {
      ...
      else
      {
        ...
        {
          Real x1 = x1_m + static_cast<Real>(n) / static_cast<Real>(pot_samples - 1) * (x1_p - x1_m);
          Real a_1, a_2, a_3;
  ...
  Real IntegratedA2(Real x1, Real x2_m, Real x2_p, Real x3)
  {
    ...
    if (field_config == MagneticFieldConfigs::density || field_config == MagneticFieldConfigs::loops)
    {
      ...
      else
      {
        ...
        for (int n = 0; n < pot_samples; ++n)
        {
          Real x2 = x2_m + static_cast<Real>(n) / static_cast<Real>(pot_samples - 1) * (x2_p - x2_m);
          Real a_1, a_2, a_3;
          VectorPotential(x1, x2, x3, &a_1, &a_2, &a_3);
  ...
  Real IntegratedA3(Real x1, Real x2, Real x3_m, Real x3_p)
  {
    ...
    if (field_config == MagneticFieldConfigs::density || field_config == MagneticFieldConfigs::loops)
    {
      ...
      else
      {
        ...
        for (int n = 0; n < pot_samples; ++n)
        {
          Real x3 = x3_m + static_cast<Real>(n) / static_cast<Real>(pot_samples - 1) * (x3_p - x3_m);
          Real a_1, a_2, a_3;
          VectorPotential(x1, x2, x3, &a_1, &a_2, &a_3);
...
namespace
{
  void VectorPotential(Real x1, Real x2, Real x3, Real *p_a_1, Real *p_a_2, Real *p_a_3)
  {
    ...
    if (field_config == MagneticFieldConfigs::loops && (r <= pot_r_min || r >= pot_r_max || th_t <= pot_theta_min || th_t >= PI - pot_theta_min))
    {
      *p_a_1 = 0.0;
      *p_a_2 = 0.0;
      *p_a_3 = 0.0;
    ...
    else if (field_config == MagneticFieldConfigs::loops)
    {
      ...
      a_r = 0.0;
      a_th_t = 0.0;
      a_ph_t = std::pow(pgas_cut, pot_pgas_pow) * SQR(r) * sth_t * std::sin(arg_r) * std::sin(arg_th);
